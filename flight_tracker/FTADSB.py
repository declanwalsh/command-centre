...
# return the classes
def parseADSB(data, origin, VERBOSE, minAngle, maxAngle):
    ...
    for entity in acListDicts:
        ...
        pos = FTModules.point(entity['Long'], entity['Lat'])

        hav = FTProcess.haversine(origin, pos) # use haversine to calculate range and bearing
        inSec = FTProcess.pointInSector(hav, minAngle, maxAngle) # check if within sector of viewing
        
        acListAdd = FTModules.aircraft(dpt, arv, mil, eng, dst, ICAO, op, mdl, alt, pos, inSec, hav.length, hav.angle, call)
